...
const ProductDetails = () => {
  ...
  const getSimilarProd = async (e) => {
    ...
    const all = await axios.get(`http://localhost:5001/allProducts`);
    ...
    const response = await axios.get(`http://localhost:5001/prod/${title}`);
    ...
    const response2 = await axios.get(
      `http://localhost:5001/recommand/${title}`
    );
